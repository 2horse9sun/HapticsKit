...
fileprivate func quadInterpolation(t: Double, numberOfPoints: Int) -> [(Double, Double)] {
    ...
    for i in 0..<numberOfPoints {
        ...
        let y = 1 - pow(1 - x / t * 2, 2)
        points.append((x, y))
    }
    return points
}

public struct HKWavePattern: HKHapticPattern {
    public let name = "Wave"
    public let detail = "A single continuous haptic event with intensity increasing to maximum and then decreasing to zero"
